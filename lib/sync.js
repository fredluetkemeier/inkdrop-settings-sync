...
async function sync() {
    ...
    console.log({
        lastSynced,
        remoteLastSynced: remote.lastSynced,
        isBefore: dayjs(lastSynced).isBefore(remote.lastSynced),
        isNotBetween: !dayjs(lastSynced).isBetween(
            dayjs(remote.lastSynced).subtract(1, "second"),
            remote.lastSynced
        ),
    });
