...
async function activate() {
    // UI Events
    cmdRef = inkdrop.commands.add(document.body, {
        [welcomeCmds.ready]: handleWelcomeReady,
        [welcomeCmds.tokenSet]: syncSettings,
    });
    ...
    // Package events
    inkdrop.packages.onDidLoadPackage(() => syncSettings());
    inkdrop.packages.onDidUnloadPackage(() => syncSettings());

    // Initial sync
    if (store.exists(keys.githubAccessToken)) syncSettings();
}

function deactivate() {
    if (cmdRef) cmdRef.dispose();
