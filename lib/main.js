...
async function setLocalSettings(changed) {
    ...
    const { packages, themes } = changed;

    const { added, removed } = packageDiff(currentPackages, packages);

    // Install added packages
    await Promise.all(
        added.map(({ name, version, isTheme }) =>
    ...
    await Promise.all(
        removed.map(({ name }) => inkdrop.ipm.uninstall({ name }))
    );

    // Disable all current themes
    currentThemes.forEach((theme) => {
        inkdrop.packages.disablePackage(theme);
    });

    // Enable all new themes
    themes.reverse().forEach(async (theme) => {
        inkdrop.packages.enablePackage(theme);
        console.log(await inkdrop.themes.getEnabledThemeNames());
    });

    // Apply updated themes to take effect in the GUI
    return inkdrop.themes.activateThemes();
}
