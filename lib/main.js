...
module.exports = {
    ...
    config: {
        syncInterval: {
            title: "Sync Interval",
            description: "Time in seconds to check for sync changes",
            type: "number",
...
let refs = [];
let isActive = false;
//let nextScheduledSync = dayjs.utc().add(5, "minutes");
...
async function activate() {
    ...
    poll(
        syncSettings,
        inkdrop.config.get("settings-sync.syncInterval") * 1000, // 5 minutes in ms
        () => !isActive
    );
}
